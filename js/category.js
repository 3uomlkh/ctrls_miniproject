...
let categoryArr = [];
export async function categoryFn() {
    ...
    try {
        ...
        console.log("categoryFn 에러 발생", err);
        return;
    }
...
export async function getCategory(large) {
    try {
        const groupCollection = collection(db, "group");
        const q = query(groupCollection, where("category","==", large));
        const querySnapShot = await getDocs(q);
        // 검색 결과가 있는 경우 해당 그룹카드를 생성하여 화면에 출력
    } catch (err) {
        console.log("getCategory 에러 발생", err);
        return;
    }
}
